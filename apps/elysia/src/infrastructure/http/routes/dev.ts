...
export function devRoutes(rt: RuntimeEnv) {
  return new Elysia().post("/api/dev/seed", async ({ request, status }) => {
    const expected = rt.env.get("DEV_SEED_TOKEN");
    if (!expected) {
      return status(500, { error: "DEV_SEED_TOKEN not set" });
    }
    ...
    try {
      ...
      if (!admin) {
        return status(500, { error: "No se pudo crear el admin" });
      }
      ...
      if (!createdCase) {
        return status(500, { error: "No se pudo crear el caso legal" });
      }
      ...
      if (!createdArticle) {
        return status(500, { error: "No se pudo crear el articulo" });
      }
