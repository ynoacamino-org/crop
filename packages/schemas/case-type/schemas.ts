...
export const createCaseTypeSchema = z.object({
  ...
  description: z.string().max(500, { message: "La descripción no puede tener más de 500 caracteres" }).optional(),
  color: z
    .string()
    .regex(/^#[0-9A-Fa-f]{6}$/, { message: "El color debe ser un código hexadecimal válido (ej: #3B82F6)" })
    .optional(),
  icon: z
    .string()
    ...
    .optional(),
  order: z.number().int().min(0, { message: "El orden debe ser un número entero no negativo" }).optional(),
